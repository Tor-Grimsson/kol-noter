...
import { useNotesStore, EXPLORER_COLORS, EXPLORER_ICONS, EditorType } from "@/store/NotesContext";
...
export type HierarchyLevel = 'root' | 'system' | 'project' | 'note' | 'page' | 'section';

export interface HierarchySelectInfo {
...
export const ExplorerSidebar = ({
  ...
  const renderSection = (section: Section, level: number, parentIds: { systemId: string; projectId: string; noteId: string; pageId: string }) => (
    <button
      key={section.id}
      onClick={() => {
        onNoteSelect(section.id);
        onHierarchySelect?.({
          level: 'section',
          id: section.id,
          parentIds
      ...
      className={cn(
        "w-full flex items-center gap-2 py-1 px-2 rounded text-left transition-colors text-sm",
  ...
  const renderPage = (page: Page, level: number, parentIds: { systemId: string; projectId: string; noteId: string }) => {
      ...
      <div key={page.id} className="w-full">
        ...
        {isExpanded && hasChildren && (
          <div className="mt-0.5 space-y-0.5">
            {page.sections.map(s => renderSection(s, level + 1, { ...parentIds, pageId: page.id }))}
          </div>
        )}
      </div>
    );
    ...
    >
          ...
          {searchExpanded ? (
            <div className="relative animate-in slide-in-from-right-2 duration-200">
              ...
              <Input
                ...
                className="h-7 w-40 text-xs bg-input border-input-border focus-visible:ring-primary pl-7 pr-7"
                autoFocus
              />
              {searchQuery && (
                <button
      ...
      <div className="p-2 border-t border-sidebar-border space-y-1">
        <Button
          ...
          className={cn(
            ...
            location.pathname === "/docs" && "bg-sidebar-active"
          )}
      ...
      <div className="p-2 border-t border-sidebar-border space-y-1">
        ...
        >
          ...
          {trash.length > 0 && (
            <span className="ml-auto text-sm text-muted-foreground">{trash.length}</span>
          )}
        </Button>
        <div className="border-t border-border my-1" />
        <Button
          variant="ghost"
          className="w-full flex items-center justify-start gap-2 py-1.5 px-2 h-auto text-sm"
