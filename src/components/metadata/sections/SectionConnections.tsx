...
import { SavedLink } from "@/store/NotesContext";
...
import { ContactCard, type Contact } from "./SectionContacts";
import {
  CELL_STYLE,
  GRID_LAYOUT,
  LABEL_STYLE,
...
export interface SectionConnectionsProps {
  tags?: string[];
      ...
      <div className={cn("space-y-4", className)}>
        ...
        {onAddTag && (
          <section>
            ...
            {/* Tag input - only shows when adding */}
            {isAddingTag && (
              <input
                type="text"
                placeholder="Add tag..."
        ...
        {onAddLink && (
          <section>
            ...
            {links.length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground mt-2">No links</p>
            )}
            {/* Link input - only shows when adding */}
            {isAddingLink && (
              <input
                type="text"
                placeholder="https://..."
                ...
                onBlur={() => {
                  if (!newLinkUrl.trim()) setIsAddingLink(false);
                }}
                className={cn(PANEL_INPUT_CLASS, "w-full mt-2")}
                autoFocus
              />
